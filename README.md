...
- 支持 `/` 开头的内部命令、可复用会话、MCP server 配置和 Skill 配置。
...
TUI 配置向导覆盖 API Key、Base URL 类型、Token Plan region、自定义 Base URL、API 格式、模型、最大输出 token、temperature、system prompt、MCP servers 与 skills。配置会写入用户配置文件：
...
当前版本提供 MCP server 与 Skill 的配置、展示和 Agent 上下文注入能力：

- `mcpServers` 保存 MCP server 的 `name`、`command`、`args`、`env`、`enabled`。
- `skills` 保存 skill 的 `name`、`path`、`description`、`enabled`。
- TUI 中 `/mcp`、`/skill` 可查看当前配置。
- Agent 每轮任务会收到已启用 MCP/Skill 列表作为系统上下文。

注意：本版本尚未启动 MCP 子进程或把 MCP tools 动态注册为可调用工具；这是后续可继续扩展的运行时集成层。
